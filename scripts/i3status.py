...
def get_fan():
    ...
    with open('/sys/devices/virtual/hwmon/hwmon3/fan1_input') as fp:
        speed = 0
        for line in fp:
            speed = line.strip()
            return speed
...
def get_speed():
    command = "/bin/bash /home/mongo/scripts/measure-net-speed.sh"
...
if __name__ == '__main__':
    ...
    while True:
        ...
        speed = get_fan()
        j.insert(2, {'full_text' : 'FAN %s RPM' % speed, 'name' : 'fan'})
        j.insert(4, {'full_text' : 'GOV %s' % get_governor(), 'name' : 'gov'})
        j.insert(5, {'full_text' : 'BRI %s%%' % get_brightness(), 'name' : 'bri'})
        j.insert(7, {'full_text' : 'WIFI %s' % get_speed(), 'name' : 'wifi'})
